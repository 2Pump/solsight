    ...
    <div className="glass relative p-5">
      ...
      {open && results.length > 0 && (
        <>
          ...
          <div
            className="fixed inset-0 z-10 bg-black/50 backdrop-blur-[1px]"
            onClick={() => setOpen(false)}
          />
          <div className="absolute left-5 right-5 top-full z-20 mt-2 max-h-72 overflow-y-auto rounded-2xl border border-border bg-surface p-1.5 shadow-2xl">
            {results.map((r) => (
              <button
                key={r.mintAddress}
